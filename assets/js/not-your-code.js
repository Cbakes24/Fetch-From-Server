...
const cbHandler = async (res) => {
  ...
  const [_, endpoint] = res.url.split("http://localhost:4000/");
...
getAllDogsButton.addEventListener("click", (e) => {
  getAllDogs().then(cbHandler).catch(errorHandler);
});

getDogTwoButton.addEventListener("click", () => {
  getDogNumberTwo().then(cbHandler).catch(errorHandler);
});

postDogsButton.addEventListener("click", () => {
  postNewDog().then(cbHandler).catch(errorHandler);
});

postDogsV2Button.addEventListener("click", () => {
  postNewDogV2(dogName.value, dogAge.value).then(cbHandler).catch(errorHandler);
});

deleteButton.addEventListener("click", () => {
  deleteDog(deleteSelect.value).then(cbHandler).catch(errorHandler);
});
